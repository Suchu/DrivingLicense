...
@section('content')
  ...
  <div id="features-wrapper">
             ...
             <section id="features" class="container">
             	@if($result)
	@if($result->applicant->status == 2)
		@if($result->expiry_date < date('Y-m-d'))
			@if(count($result->renews))
				<?php $renewcounts = count($result->renews);  $renew = $result->renews;?>
				@if($renew[$renewcounts-1]->new_expiry < date('Y-m-d'))
					<h3>Expired!</h3><p>Please Renew your license by filling the voucher payment details:</p><br/>
	          	 	<a href="{{url('renew/revoucher/'.$result->applicant_id)}}">Please click here:</a>
				@else
					Expiry Date: {{date('Y-m-d', strtotime($renew[$renewcounts-1]->new_expiry))}}
				@endif
			@else
				Expired so renew please by filling the voucher payment details:<br/>
	          	 <a href="{{url('renew/revoucher/'.$result->applicant_id)}}">Please click here:</a>
			@endif
		@else
			Expiry Date: {{date('Y-m-d', strtotime($result->expiry_date))}}
		@endif
	@else
		<p>Not passed yet.</p>
	@endif
